...
def greedy_decode(
    ...
    # Pre-compute the encoder output and reuse it for every token we get from the encoder
    ...
    while True:
        ...
        # Select the token id with the highest probability (because it is a greedy search)
...
def run_validaton(
    ...
    # Size of the control window (just use a defualt window)
    console_width = 80
    ...
    with torch.no_grad():
        for batch in validation_ds:
            ...
            assert encoder_inp.size(0) == 1, "Batch size must be one for validation"
            ...
            print_msg(f"EXPECTED: {model_output_text}")
...
def train_model(config):
    ...
    for epoch in range(initial_epoch, config["num_epochs"]):
        ...
        for batch in batch_iterator:
            ...
            # Compute the loss (batch_size, seq_len, vocab_size) --> (batch_size * seq_len, vocab_size)
            loss = loss_fn(
